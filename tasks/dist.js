...
module.exports = function (gulp, distPaths, outPath) {
  ...
  gulp.task('dist:core', function () {
    ...
    return gulp.src(distPaths)
      .pipe(sourcemap.init())
      .pipe(insert.prepend('/*! liten v' + tag + ' (c) ' + year + ' Sam Richard, MIT license */\n'))
      .pipe(rename({
        extname: '.min.js'
      }))
      .pipe(uglify({
  ...
  gulp.task('dist:polyfill', function () {
    ...
    return gulp.src(distPaths)
      ...
      .pipe(insert.prepend('/*! liten (with polyfills) v' + tag + ' (c) ' + year + ' Sam Richard with thanks to the Financial Times, MIT license */\n'))
      .pipe(rename({
  ...
  gulp.task('dist:polyfills', function () {
    ...
    return string_src('polyfills.min.js', polyfills)
      .pipe(sourcemap.init())
      .pipe(insert.prepend('/*! liten polyfills v' + tag + ' (c) ' + year + ' Sam Richard with thanks to the Financial Times, MIT license */\n'))
      .pipe(uglify({
        preserveComments: 'some'
      }))
      .pipe(sourcemap.write('./'))
